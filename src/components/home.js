  ...
  return (
    <div className="bg-gradient-to-br from-white via-slate-100 to-slate-200 text-gray-800 min-h-screen font-sans">
      ...
      <FadeInSection delay={0.1}>
        <section id="about" className="bg-gradient-to-b from-white to-indigo-50 py-24 px-6">
      ...
      {/* ... Gallery, Modal, Testimonials, Contact, and Footer sections remain unchanged with fade-in wrapping each major section ... */}

      <FadeInSection>
        {/* Gallery */}
        <section id="gallery" className="bg-gradient-to-b from-white to-gray-100 py-24 px-6">
      ...
      <FadeInSection>
        ...
        <section id="contact" className="py-24 px-6 bg-white">
          <div className="max-w-3xl mx-auto bg-gray-50 p-8 rounded-2xl shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="Phone" required className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="Email Address" required className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
              </div>
              <textarea name="message" value={formData.message} onChange={handleInputChange} placeholder="Message" rows={5} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"></textarea>
              <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-semibold shadow-md">
                {loading ? "Sending..." : "Send Message"}
              </button>
              {success && <p className="text-green-600 text-center mt-2">✅ Message sent! We will get back shortly.</p>}
            </form>
          </div>
        </section>
      </FadeInSection>
      ...
      <footer className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white py-12 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
          <div>
            <h4 className="text-xl font-bold mb-4">R K Balloons</h4>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li><a href="https://wa.me/918688810980" target="_blank" rel="noreferrer" className="hover:underline">WhatsApp</a></li>
            </ul>
          </div>
          <div>
        ...
        <p className="text-center text-sm text-gray-400 relative z-10">© {new Date().getFullYear()} R K Balloons & Decorations. All rights reserved.</p>
      </footer>


      <div className="fixed bottom-6 right-6 flex flex-col gap-4 z-50">
        <a href="https://wa.me/918688810980" target="_blank" rel="noopener noreferrer" className="bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-xl transition-all">
          <FaWhatsapp className="w-6 h-6" />
        </a>
        <a href="https://www.instagram.com/your_instagram_username" target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-pink-500 via-red-500 to-yellow-500 hover:opacity-90 text-white p-3 rounded-full shadow-xl transition-all">
